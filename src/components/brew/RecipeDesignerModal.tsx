...
export default function RecipeDesignerModal({
  ...
  const [activeTab, setActiveTab] = useState<'FERMENTABLES' | 'HOPS' | 'YEAST' | 'WATER' | 'MASH' | 'COST'>('FERMENTABLES');
  ...
  useEffect(() => {
    if (recipe) {
      ...
      // Receita Nova Padrão: American IPA 500L
      setFermentables([
  ...
  const handleSaveRecipe = async () => {
    ...
    try {
      // Converte insumos para formato da tabela RecipeIngredient
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
      <div className="bg-slate-900 border border-slate-700/80 rounded-3xl w-full max-w-6xl text-slate-100 shadow-2xl flex flex-col max-h-[94vh] overflow-hidden">
        {/* HEADER & COCKPIT */}
        <div className="p-4 sm:p-6 border-b border-slate-800 bg-slate-950/60">
          <div className="flex items-center justify-between pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Calculadora físico-química e designer de receitas estilo Brewfather
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-xl transition-all"
          ...
          {/* REAL-TIME BREWING COCKPIT (GAUGES & METRICS) */}
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 pt-2">
            {/* OG & Plato */}
            <div className="p-3 bg-slate-900/90 rounded-2xl border border-slate-800/80 flex flex-col justify-between">
              <div className="flex items-center justify-between text-[11px] font-bold text-slate-400">
                <span>OG (Densidade)</span>
                <span className="text-amber-400 font-black">{plato}°P</span>
              </div>
              ...
              {currentBjcpStyle && (
                <div className="text-[10px] text-slate-400 flex items-center gap-1 mt-1">
                  <span>BJCP:</span>
                  <span className={compliance?.isOgOk ? 'text-emerald-400 font-bold' : 'text-amber-400 font-bold'}>
            ...
            <div className="p-3 bg-slate-900/90 rounded-2xl border border-slate-800/80 flex flex-col justify-between">
              <div className="flex items-center justify-between text-[11px] font-bold text-slate-400">
                ...
                <span className="text-cyan-400 font-black">{yeast?.attenuationPercent || 78}% Aten.</span>
              </div>
              <div className="text-xl sm:text-2xl font-black text-white mt-1">
                {fg.toFixed(3)}
              </div>
              {currentBjcpStyle && (
                <div className="text-[10px] text-slate-400 flex items-center gap-1 mt-1">
                  <span>BJCP:</span>
                  <span className={compliance?.isFgOk ? 'text-emerald-400 font-bold' : 'text-amber-400 font-bold'}>
            ...
            <div className="p-3 bg-slate-900/90 rounded-2xl border border-slate-800/80 flex flex-col justify-between">
              ...
              {currentBjcpStyle && (
                <div className="text-[10px] text-slate-400 flex items-center gap-1 mt-1">
                  <span>BJCP:</span>
                  <span className={compliance?.isAbvOk ? 'text-emerald-400 font-bold' : 'text-amber-400 font-bold'}>
            ...
            <div className="p-3 bg-slate-900/90 rounded-2xl border border-slate-800/80 flex flex-col justify-between">
              <div className="flex items-center justify-between text-[11px] font-bold text-slate-400">
                ...
                <span className="text-[10px] font-bold text-slate-500">Tinseth</span>
              </div>
              ...
              {currentBjcpStyle && (
                <div className="text-[10px] text-slate-400 flex items-center gap-1 mt-1">
                  <span>BJCP:</span>
                  <span className={compliance?.isIbuOk ? 'text-emerald-400 font-bold' : 'text-amber-400 font-bold'}>
            ...
            <div className="p-3 bg-slate-900/90 rounded-2xl border border-slate-800/80 flex flex-col justify-between">
              <div className="flex items-center justify-between text-[11px] font-bold text-slate-400">
                <span>Cor (EBC / SRM)</span>
                <span className="text-[10px] font-bold text-slate-400">{colorData.srm} SRM</span>
              </div>
              <div className="flex items-center gap-2 mt-1">
                <div
                  className="w-5 h-7 rounded-md border border-slate-700 shadow-inner"
                  style={{ backgroundColor: colorData.hexColor }}
                  title={`Cor calculada: ${colorData.ebc} EBC (${colorData.srm} SRM)`}
                />
                <span className="text-xl sm:text-2xl font-black text-white">
                  {colorData.ebc} <span className="text-xs font-bold text-slate-500">EBC</span>
                </span>
              </div>
              {currentBjcpStyle && (
                <div className="text-[10px] text-slate-400 flex items-center gap-1 mt-1">
                  <span>BJCP:</span>
                  <span className={compliance?.isEbcOk ? 'text-emerald-400 font-bold' : 'text-amber-400 font-bold'}>
            ...
            {/* EQUILÍBRIO BU:GU & CUSTO */}
            <div className="p-3 bg-slate-900/90 rounded-2xl border border-slate-800/80 flex flex-col justify-between">
              <div className="flex items-center justify-between text-[11px] font-bold text-slate-400">
                <span>Relação BU:GU</span>
                <span className="text-xs font-black text-purple-400">{bugu.ratio.toFixed(2)}</span>
              </div>
              ...
              <div className="text-[10px] text-amber-400 font-bold mt-1 flex items-center justify-between">
                <span>CPV:</span>
                <span className="font-black text-xs text-white">R$ {costPerLiter.toFixed(2)}/L</span>
              </div>
            </div>
          </div>
        </div>

        {/* CONTROLES DE RECEITA & NAVEGAÇÃO DE ABAS */}
        <div className="px-4 sm:px-6 pt-4 pb-2 border-b border-slate-800 bg-slate-900 flex flex-wrap items-center justify-between gap-3">
          {/* Inputs Básicos */}
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full lg:w-auto">
            ...
            <div className="w-full sm:w-60">
              <select
                ...
                className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-xl text-xs font-bold text-amber-300 focus:outline-none focus:border-amber-500"
              >
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1.5 bg-slate-950 px-2.5 py-1.5 border border-slate-700 rounded-xl text-xs">
                <span className="text-slate-400 font-bold">Lote:</span>
                <input
                  type="number"
                  value={batchVolumeLiters}
                  onChange={(e) => setBatchVolumeLiters(Math.max(10, parseFloat(e.target.value) || 0))}
                  className="w-16 bg-transparent text-white font-black text-right focus:outline-none"
                />
                <span className="text-slate-400">L</span>
              </div>

              <div className="flex items-center gap-1.5 bg-slate-950 px-2.5 py-1.5 border border-slate-700 rounded-xl text-xs">
                <span className="text-slate-400 font-bold">Efic:</span>
                <input
                  type="number"
                  value={efficiencyPercent}
                  onChange={(e) => setEfficiencyPercent(Math.max(10, Math.min(100, parseFloat(e.target.value) || 0)))}
                  className="w-12 bg-transparent text-white font-black text-right focus:outline-none"
                />
          ...
          {/* Abas */}
          <div className="flex items-center gap-1 bg-slate-950 p-1 rounded-2xl border border-slate-800 overflow-x-auto w-full lg:w-auto">
            <button
              onClick={() => setActiveTab('FERMENTABLES')}
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 whitespace-nowrap ${
                activeTab === 'FERMENTABLES' ? 'bg-amber-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 whitespace-nowrap ${
                activeTab === 'HOPS' ? 'bg-emerald-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 whitespace-nowrap ${
                activeTab === 'YEAST' ? 'bg-cyan-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 whitespace-nowrap ${
                activeTab === 'WATER' ? 'bg-blue-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'
              }`}
            >
              <Beaker className="w-3.5 h-3.5" />
              <span>Perfil de Água</span>
            </button>

            <button
              onClick={() => setActiveTab('MASH')}
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 whitespace-nowrap ${
                activeTab === 'MASH' ? 'bg-orange-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'
              }`}
            >
              <Thermometer className="w-3.5 h-3.5" />
              <span>Mostura & Volumes</span>
            </button>

            <button
              onClick={() => setActiveTab('COST')}
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 whitespace-nowrap ${
                activeTab === 'COST' ? 'bg-purple-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'
              }`}
            >
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-900/60">
          {error && (
            <div className="mb-4 p-3 bg-rose-500/20 border border-rose-500/40 text-rose-300 rounded-xl text-xs font-bold flex items-center gap-2">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs rounded-xl flex items-center gap-1.5 transition-all shadow-md"
                >
                ...
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-950/80 text-slate-400 font-bold border-b border-slate-800">
                    <tr>
                      ...
                      <th className="p-3">Estoque PintTech</th>
                        ...
                        <tr key={idx} className="hover:bg-slate-800/40 transition-colors">
                          <td className="p-3">
                            <input
                              ...
                              className="w-full bg-slate-950 border border-slate-700/80 rounded-lg px-2.5 py-1.5 text-xs text-white font-bold focus:outline-none focus:border-amber-500"
                            />
                          </td>
                          <td className="p-3">
                            <input
                              ...
                              className="w-full bg-slate-950 border border-slate-700/80 rounded-lg px-2.5 py-1.5 text-xs text-amber-300 font-black focus:outline-none focus:border-amber-500 text-right"
                            />
                          </td>
                          ...
                          <td className="p-3">
                            <input
                              ...
                              className="w-full bg-slate-950 border border-slate-700/80 rounded-lg px-2 py-1.5 text-xs text-slate-200 font-bold focus:outline-none text-right"
                            />
                          </td>
                          <td className="p-3">
                            <input
                          ...
                          <td className="p-3">
                            <input
                              ...
                              className="w-full bg-slate-950 border border-slate-700/80 rounded-lg px-2 py-1.5 text-xs text-emerald-400 font-bold focus:outline-none text-right"
                            />
                          </td>
                          <td className="p-3">
                            {stock ? (
                              ...
                              <span className="text-[11px] text-slate-500 italic">Não vinculado</span>
                            )}
                          </td>
              ...
              <div className="p-4 bg-slate-950/60 rounded-2xl border border-slate-800">
                <p className="text-xs font-bold text-slate-400 mb-2">Adição Rápida do Catálogo Comercial:</p>
                <div className="flex flex-wrap gap-2">
                  {POPULAR_MALTS.slice(0, 10).map((m, idx) => (
                    <button
                      ...
                      className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-lg text-xs font-bold transition-all flex items-center gap-1"
                    >
                      <span>+ {m.name}</span>
                      <span className="text-[10px] text-amber-400">({m.colorEbc} EBC)</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Cálculo Tinseth com suporte a First Wort, Whirlpool térmico (decay de isomerização) e Dry Hopping.
                  </p>
                </div>
                ...
                <button
                  type="button"
                  onClick={() => addHop()}
                  className="px-3 py-1.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs rounded-xl flex items-center gap-1.5 transition-all shadow-md"
                >
                  <Plus className="w-4 h-4" />
                        ...
                        <tr key={idx} className="hover:bg-slate-800/40 transition-colors">
                          <td className="p-3">
                            <input
                              ...
                              className="w-full bg-slate-950 border border-slate-700/80 rounded-lg px-2.5 py-1.5 text-xs text-white font-bold focus:outline-none focus:border-emerald-500"
                            />
                          </td>
                          <td className="p-3">
                            <input
                              ...
                              className="w-full bg-slate-950 border border-slate-700/80 rounded-lg px-2.5 py-1.5 text-xs text-emerald-300 font-black focus:outline-none focus:border-emerald-500 text-right"
                            />
                          </td>
                          <td className="p-3">
                            <input
                          ...
                          <td className="p-3">
                            <select
                              ...
                              className="w-full bg-slate-950 border border-slate-700/80 rounded-lg px-2 py-1.5 text-xs text-amber-300 font-bold focus:outline-none"
                            >
                          ...
                          <td className="p-3">
                            <input
                              ...
                              className="w-full bg-slate-950 border border-slate-700/80 rounded-lg px-2 py-1.5 text-xs text-purple-300 font-bold focus:outline-none text-right"
                            />
                          </td>
                          <td className="p-3">
                            {stock ? (
              ...
              <div className="p-4 bg-slate-950/60 rounded-2xl border border-slate-800">
                <p className="text-xs font-bold text-slate-400 mb-2">Adição Rápida de Lúpulos Nobres & Modernos:</p>
                <div className="flex flex-wrap gap-2">
                  {POPULAR_HOPS.map((hp, idx) => (
                    ...
                    >
                      <span>+ {hp.name}</span>
                      <span className="text-[10px] text-emerald-400">({hp.alphaAcidPercent}% AA)</span>
                    </button>
                  ))}
                </div>
              </div>
          ...
          {activeTab === 'YEAST' && (
            <div className="space-y-6 max-w-2xl">
              ...
              <div className="p-5 bg-slate-950 rounded-2xl border border-slate-800 space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1">Cepa / Nome da Levedura</label>
                  <input
                    type="text"
                    value={yeast?.name || ''}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1">Atenuação Aparente (%)</label>
                    <input
                      type="number"
                      ...
                      className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-sm text-cyan-400 font-black focus:outline-none"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1">Temp. Mínima (°C)</label>
                    <input
                      type="number"
                      value={yeast?.minTempCelsius || 18}
                      onChange={(e) => setYeast({ ...yeast, minTempCelsius: parseFloat(e.target.value) || 18 })}
                      className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-200 font-bold focus:outline-none"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1">Temp. Máxima (°C)</label>
                    <input
                      type="number"
              ...
              <div className="p-4 bg-slate-950/60 rounded-2xl border border-slate-800">
                <p className="text-xs font-bold text-slate-400 mb-2">Selecionar Cepas Populares:</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {POPULAR_YEASTS.map((y, idx) => (
                    ...
                    >
                      <span>{y.name}</span>
                      <span className="text-[10px] text-cyan-400 font-bold">{y.attenuationPercent}%</span>
                    </button>
                  ))}
                </div>
              </div>
          ...
          {activeTab === 'WATER' && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Ajuste o balanço de Sulfato e Cloreto para realçar o amargor seco ou a maciez e suculência da cerveja.
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-6 gap-3">
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-center">
                  <span className="text-[10px] font-bold text-slate-400">Cálcio (Ca²⁺)</span>
                  <div className="text-lg font-black text-white mt-1">{adjustedWater.ca} <span className="text-[10px] text-slate-500">ppm</span></div>
                </div>
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-center">
                  <span className="text-[10px] font-bold text-slate-400">Magnésio (Mg²⁺)</span>
                  <div className="text-lg font-black text-white mt-1">{adjustedWater.mg} <span className="text-[10px] text-slate-500">ppm</span></div>
                </div>
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-center">
                  <span className="text-[10px] font-bold text-slate-400">Sódio (Na⁺)</span>
                  <div className="text-lg font-black text-white mt-1">{adjustedWater.na} <span className="text-[10px] text-slate-500">ppm</span></div>
                </div>
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-center">
                  <span className="text-[10px] font-bold text-slate-400">Sulfato (SO₄²⁻)</span>
                  <div className="text-lg font-black text-amber-400 mt-1">{adjustedWater.so4} <span className="text-[10px] text-slate-500">ppm</span></div>
                </div>
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-center">
                  <span className="text-[10px] font-bold text-slate-400">Cloreto (Cl⁻)</span>
                  <div className="text-lg font-black text-cyan-400 mt-1">{adjustedWater.cl} <span className="text-[10px] text-slate-500">ppm</span></div>
                </div>
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-center">
                  <span className="text-[10px] font-bold text-slate-400">Bicarbonato</span>
                  <div className="text-lg font-black text-slate-300 mt-1">{adjustedWater.hco3} <span className="text-[10px] text-slate-500">ppm</span></div>
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 flex items-center justify-between">
                <div>
                  <span className="text-xs font-bold text-slate-400">Relação SO₄ : Cl</span>
                  <div className="text-lg font-black text-white mt-0.5">
                    {adjustedWater.sulfateChlorideRatio.toFixed(2)} : 1
                  </div>
                </div>
                <div className="px-3 py-1.5 rounded-xl bg-blue-500/20 text-blue-300 border border-blue-500/30 text-xs font-bold">
                  {adjustedWater.profileCharacter}
                </div>
              </div>

              {/* Adições de Sais em Gramas */}
              <div className="p-5 bg-slate-950 rounded-2xl border border-slate-800 space-y-4">
                <h4 className="text-xs font-black text-white uppercase tracking-wider">Adição de Sais Cervejeiros (Gramas Totais)</h4>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 mb-1">Gipsita / CaSO₄ (g)</label>
                    <input
                      type="number"
                      value={salts.gypsumGrams}
                      onChange={(e) => setSalts({ ...salts, gypsumGrams: Math.max(0, parseFloat(e.target.value) || 0) })}
                      className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs font-black text-amber-300 focus:outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 mb-1">Cloreto de Cálcio / CaCl₂ (g)</label>
                    <input
                      type="number"
                      value={salts.calciumChlorideGrams}
                      onChange={(e) => setSalts({ ...salts, calciumChlorideGrams: Math.max(0, parseFloat(e.target.value) || 0) })}
                      className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs font-black text-cyan-300 focus:outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 mb-1">Sal de Epsom / MgSO₄ (g)</label>
                    <input
                      type="number"
                      value={salts.epsomSaltGrams}
                      onChange={(e) => setSalts({ ...salts, epsomSaltGrams: Math.max(0, parseFloat(e.target.value) || 0) })}
                      className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs font-black text-slate-200 focus:outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 mb-1">Sal de Cozinha / NaCl (g)</label>
                    <input
                      type="number"
                ...
                <div className="pt-3 border-t border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Droplets className="w-4 h-4 text-emerald-400" />
                    <span className="text-xs text-slate-300 font-bold">Acidificação Estimada da Mostura (pH ~5.3):</span>
                  </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800">
                  <span className="text-xs font-bold text-slate-400">Água de Mostura</span>
                  <div className="text-2xl font-black text-amber-400 mt-1">
                    {mashVolumes.mashWaterLiters} <span className="text-xs font-bold text-slate-500">Litros</span>
                  </div>
                  <p className="text-[11px] text-slate-400 mt-1">Relação: {mashRatio} L/kg</p>
                </div>

                <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800">
                  <span className="text-xs font-bold text-slate-400">Temp. de Arriação (Strike)</span>
                  <div className="text-2xl font-black text-orange-400 mt-1">
                    {mashVolumes.strikeTempCelsius}°C
                  </div>
                  <p className="text-[11px] text-slate-400 mt-1">Alvo na panela: {targetMashTemp}°C</p>
                </div>

                <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800">
                  <span className="text-xs font-bold text-slate-400">Água de Lavagem (Sparge)</span>
                  <div className="text-2xl font-black text-cyan-400 mt-1">
                    {mashVolumes.spargeWaterLiters} <span className="text-xs font-bold text-slate-500">Litros</span>
                  </div>
                  <p className="text-[11px] text-slate-400 mt-1">Aquecer a 76°C - 78°C</p>
                </div>

                <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800">
                  <span className="text-xs font-bold text-slate-400">Volume Pré-Fervura</span>
                  <div className="text-2xl font-black text-white mt-1">
                    {mashVolumes.preBoilVolumeLiters} <span className="text-xs font-bold text-slate-500">Litros</span>
                  </div>
                  <p className="text-[11px] text-slate-400 mt-1">Evaporação: {mashVolumes.boilOffLiters}L ({boilTimeMinutes}m)</p>
                </div>
              </div>
              ...
              <div className="p-5 bg-slate-950 rounded-2xl border border-slate-800 space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-black text-white uppercase tracking-wider">Rampas de Mostura (Mash Schedule)</h4>
                  <button
                    type="button"
                    onClick={() => setMashSteps([...mashSteps, { name: 'Nova Rampa', type: 'TEMPERATURE', stepTempCelsius: 72, stepTimeMinutes: 15 }])}
                    className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-xs font-bold text-slate-200 rounded-lg"
                  >
                    + Rampa
                  </button>
                </div>
                ...
                <div className="space-y-2">
                  {mashSteps.map((step, idx) => (
                    <div key={idx} className="flex items-center gap-3 p-3 bg-slate-900 rounded-xl border border-slate-800">
                      <div className="w-7 h-7 rounded-lg bg-orange-500/20 text-orange-400 font-black text-xs flex items-center justify-center">
                        {idx + 1}
                      </div>
                      <input
                        type="text"
                        ...
                        className="flex-1 bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1 text-xs text-white font-bold focus:outline-none"
                      />
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          className="w-16 bg-slate-950 border border-slate-700 rounded-lg px-2 py-1 text-xs text-orange-300 font-black text-right focus:outline-none"
                        />
                      ...
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          className="w-14 bg-slate-950 border border-slate-700 rounded-lg px-2 py-1 text-xs text-slate-200 font-bold text-right focus:outline-none"
                        />
          ...
          {activeTab === 'COST' && (
            <div className="space-y-6 max-w-3xl">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800">
                  <span className="text-xs font-bold text-slate-400">Custo Total de Insumos</span>
                  <div className="text-2xl font-black text-purple-400 mt-1">
                    {formatCurrency(totalCost)}
                  </div>
                  <p className="text-[11px] text-slate-400 mt-1">Para lote de {batchVolumeLiters}L</p>
                </div>

                <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800">
                  <span className="text-xs font-bold text-slate-400">Custo por Litro (CPV)</span>
                  <div className="text-2xl font-black text-emerald-400 mt-1">
                    {formatCurrency(costPerLiter)}/L
                  </div>
                  <p className="text-[11px] text-slate-400 mt-1">Custo direto de fabricação</p>
                </div>

                <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800">
                  <span className="text-xs font-bold text-slate-400">Preço Sugerido de Venda</span>
                  <div className="text-2xl font-black text-white mt-1">
                    {formatCurrency(salePricePerLiter)}/L
                  </div>
                  <p className="text-[11px] text-slate-400 mt-1">Margem: {profitMarginPercent}%</p>
                </div>
              </div>
              ...
              <div className="p-5 bg-slate-950 rounded-2xl border border-slate-800 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1">Nº Registro MAPA do Rótulo</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1">Denominação Comercial Legal</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1">Notas Sensoriais & Descrição</label>
                  <textarea
                    rows={3}
                    value={description}
                    onChange={(e) => setDescription(e.target.value)}
                    placeholder="Aromas de frutas tropicais, notas cítricas, amargor limpo e final seco..."
                    className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs font-medium text-slate-200 focus:outline-none focus:border-amber-500"
                  />
                </div>
              </div>
            </div>
        ...
        <div className="p-4 sm:p-6 border-t border-slate-800 bg-slate-950 flex items-center justify-between">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2.5 text-xs font-bold text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-all"
          >
            Cancelar
          </button>
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handleExportXml}
              className="sm:hidden px-3 py-2 text-xs font-bold text-slate-300 bg-slate-800 rounded-xl"
            >
              XML
            </button>
